...
export default class RpSwiper extends BaseVue {
  @Prop({
    required: true,
    type: Number
  })
  ...
  renderSwiper(renderSlideFunc) {
    return (
      <div class="swiper-container" ref="swiper" style={{ width: this.width + "px", height: this.height + "px", background: this.background }}>
        <div class="swiper-wrapper">{renderSlideFunc && renderSlideFunc()}</div>
      </div>
    );
  }

  renderSlide(): VNode[] | null {
    return null;
  }

  render() {
    return this.renderSwiper(this.renderSlide);
  }
}
</script>
